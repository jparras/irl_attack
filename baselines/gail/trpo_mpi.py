...
def traj_segment_generator(pi, env, reward_giver, horizon, stochastic):
    ...
    true_rews = np.zeros([horizon, n_agents], 'float32')
    ...
    while True:
        prevac = ac
        ac = np.zeros_like(prevac)
        vpred = np.zeros(n_agents)
        for ag in range(n_agents):
            ac[ag], vpred[ag] = pi.act(stochastic, ob[ag])
        # Slight weirdness here because we need value function at time T
        # before returning segment [0, T-1] so we get the correct
        # terminal value
        if t > 0 and t % horizon == 0:
            ...
            for ag in range(n_agents):
                _, vpred[ag] = pi.act(stochastic, ob[ag])
            # Be careful!!! if you change the downstream algorithm to aggregate
            # several of these batches, then be sure to do a deepcopy
            ep_rets = []
            ep_true_rets = []
        ...
        rew = np.squeeze(np.array([reward_giver.get_reward(ob[ag], ac[ag]) for ag in range(n_agents)]))
        ob, true_rew, new, _ = env.step(ac)
        rews[i] = rew
        true_rews[i] = true_rew
...
def learn(env, policy_func, reward_giver, expert_dataset, rank,
    ...
    # ----------------------------------------
    ob_space = env.observation_space_def  # Careful here: we are using two different obs spaces!! This is for defense!
    ac_space = env.action_space
    #if pretrained_weight != None:
    #    pi = policy_func("pi", ob_space, ac_space, reuse=tf.AUTO_REUSE)
    #else:
    ...
    if pretrained_weight is not None:
        ...
        saver = tf.train.Saver(var_list=pi.get_variables())  # Load only variables under the policy scope
        saver.restore(tf.get_default_session(), pretrained_weight)
        print('Successfully preloaded weights')
    ...
    while True:
        ...
        for ob_batch, ac_batch in dataset.iterbatches((ob, ac),
            ...
            ob_expert = np.squeeze(ob_expert)
        ...
        lrlocal = (seg[0]["ep_lens"], seg[0]["ep_rets"], seg[0]["ep_true_rets"])  # local values
        ...
        info.append({'EpLenMean': np.mean(lenbuffer),
                     "EpRewMean": np.mean(rewbuffer),
                     "EpThisIter": len(lens),
                     "EpTrueRewMean": np.mean(true_rewbuffer)})
    return pi, reward_giver, info
