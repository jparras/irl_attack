...
class TransitionClassifier(object):
    def __init__(self, obs_space, action_space, hidden_size, entcoeff=0.001, lr_rate=1e-3, scope="adversary"):
        self.scope = scope
        ...
        if isinstance(action_space, spaces.Box):
            ...
            raise RuntimeError('Action space not recognized')
    ...
    def get_reward(self, obs, acs):
        ...
        if len(acs.shape) == 0:  # Needed for 1 D actions
            acs = np.array([acs])
        if len(acs.shape) == 1:
    ...
    def load_model(self, dir, name):
        U.initialize()
        saver = tf.train.Saver(var_list=self.get_variables())  # Load only variables under the adversary scope
